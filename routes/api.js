...
router.get('/api/instances/suspend', validateApiKey, async (req, res) => {
  ...
  try {
    ...
    let instanceToSuspend = instances.find(obj => obj.InstanceId === instance.InstanceId);
...
router.get('/api/instances/unsuspend', validateApiKey, async (req, res) => {
  ...
  try {
    ...
    let instanceToUnsuspend = instances.find(obj => obj.InstanceId === instance.InstanceId);
...
async function deleteInstance(instance) {
  try {
    await axios.get(`http://Skyport:${instance.Node.apiKey}@${instance.Node.address}:${instance.Node.port}/instances/${instance.InstanceId}/delete`);
    
    // Update user's instances
    let userInstances = await db.get(instance.User + '_instances') || [];
    userInstances = userInstances.filter(obj => obj.InstanceId !== instance.InstanceId);
    await db.set(instance.User + '_instances', userInstances);
    
    // Update global instances
    let globalInstances = await db.get('instances') || [];
    globalInstances = globalInstances.filter(obj => obj.InstanceId !== instance.InstanceId);
    await db.set('instances', globalInstances);
    
    // Delete instance-specific data
    await db.delete(instance.InstanceId + '_instance');
  } catch (error) {
    console.error(`Error deleting instance ${instance.InstanceId}:`, error);
    throw error;
  }
}
...
async function checkNodeStatus(node) {
  try {
    ...
    const response = await axios(RequestData);
    const { versionFamily, versionRelease, online, remote, hydraDaemon } = response.data;

    node.status = 'Online';
    node.versionFamily = versionFamily;
    node.versionRelease = versionRelease;
    node.remote = remote;
    node.hydraDaemon = hydraDaemon;
