...
router.get('/instances/startup/changeimage/:id', async (req, res) => {
    ...
    try {
        ...
        const requestData = await prepareRequestData(image, instance.Memory, instance.Cpu, instance.Ports, instance.Name, node, id, instance.InstanceId, instance.Env);
...
async function prepareRequestData(image, memory, cpu, ports, name, node, id, instanceId, Env) {
    ...
    const requestData = {
        method: 'post',
        url: `http://${node.address}:${node.port}/instances/redeploy/${instanceId}`,
        auth: {
            username: 'Skyport',
