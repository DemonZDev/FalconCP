...
router.post('/instance/reinstall/:id', async (req, res) => {
    ...
    try {
        ...
        const { Node: node, imageData, Memory: memory, Cpu: cpu, Ports: ports, Name: name, User: user, Primary: primary, InstanceId: instanceId, Env } = instance;
        ...
        const requestData = await prepareRequestData(shortimage, memory, cpu, ports, name, node, id, instanceId, Env);
        ...
        res.status(500).json({
            error: 'Failed to reinstall instance',
...
async function prepareRequestData(image, memory, cpu, ports, name, node, id, instanceId, Env) {
    const rawImages = await db.get('images');
    const imageData = rawImages.find(i => i.Image === image);

    const requestData = {
        method: 'post',
        url: `http://${node.address}:${node.port}/instances/reinstall/${instanceId}`,
        auth: {
            username: 'Skyport',
