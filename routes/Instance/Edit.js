...
router.put('/instances/edit/:id', isAdmin, async (req, res) => {
  ...
  try {
    ...
    const updatedInstance = await updateInstanceInDatabase(id, instance, Image, Memory, Cpu, response.data.newInstanceId);
    ...
    res.status(200).json({
      ...
      oldInstanceId: id,
      newInstanceId: response.data.newInstanceId
    });
...
function prepareEditRequestData(instance, Image, Memory, Cpu) {
  ...
  return {
    method: 'put',
    url: `http://${node.address}:${node.port}/instances/edit/${instance.InstanceId}`,
    auth: {
      username: 'Skyport',
...
async function updateInstanceInDatabase(id, instance, Image, Memory, Cpu, newInstanceId) {
  ...
  await db.set(`${newInstanceId}_instance`, updatedInstance);
...
async function updateUserInstances(userId, oldInstanceId, updatedInstance) {
  const userInstances = await db.get(`${userId}_instances`) || [];
  const updatedUserInstances = userInstances.map(inst => 
    inst.InstanceId === oldInstanceId ? {...inst, ...updatedInstance} : inst
  );
  await db.set(`${userId}_instances`, updatedUserInstances);
}

async function updateGlobalInstances(oldInstanceId, updatedInstance) {
