 ...
 * @param {string} instanceId - The container ID to check authorization for.
 * @returns {Promise<boolean>} True if the user is authorized, otherwise false.
 */
async function isUserAuthorizedForContainer(userId, instanceId) {
    try {
        ...
        const isInSubUserInstances = subUserInstances.includes(instanceId);

        const isInUserInstances = userInstances.some(instance => instance.Id === instanceId);
        if (isInSubUserInstances || isInUserInstances) {
            return true;
        } else {
            console.error('User not authorized for container:', instanceId);
            return false;
        }
    } catch (error) {
